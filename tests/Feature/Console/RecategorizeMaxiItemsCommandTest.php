...
class RecategorizeMaxiItemsCommandTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_applies_rules_to_existing_items(): void
    {
        ...
        $item = MaxiReceiptItem::factory()->create(['name' => 'Jogurt MM 1kg', 'product_category_id' => null]);

        $this->artisan('maxi:recategorize-items')->assertSuccessful();
    ...
    public function test_dry_run_changes_nothing(): void
    {
        $dairy = ProductCategory::factory()->create();
        ProductCategoryRule::factory()->for($dairy)->create(['pattern' => 'jogurt']);
        $item = MaxiReceiptItem::factory()->create(['name' => 'Jogurt MM', 'product_category_id' => null]);

        $this->artisan('maxi:recategorize-items --dry-run')->assertSuccessful();
    ...
    public function test_skips_manually_categorized_items_unless_forced(): void
    {
        ...
        $item = MaxiReceiptItem::factory()->create([
            'name' => 'Jogurt MM',
        ...
        $this->artisan('maxi:recategorize-items --force')->assertSuccessful();
