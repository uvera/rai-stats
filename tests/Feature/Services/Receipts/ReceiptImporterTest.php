...
class ReceiptImporterTest extends TestCase
{
    ...
    private function fetched(): FetchedReceipt
    {
        $pdf = $this->pdf();

        return new FetchedReceipt((new ReceiptPdfParser)->parse($pdf), $pdf);
    }
    ...
    public function test_imports_receipt_with_parsed_items(): void
    {
        ...
        $receipt = (new ReceiptImporter)->import($account, $this->summary(), $this->fetched());
    ...
    public function test_reimporting_the_same_invoice_is_idempotent(): void
    {
        ...
        $importer->import($account, $this->summary(), $this->fetched());
        $importer->import($account, $this->summary(), $this->fetched());

        $this->assertDatabaseCount('grocery_receipts', 1);
        $this->assertDatabaseCount('grocery_receipt_items', 8);
    }
    ...
    public function test_auto_links_a_matching_bank_transaction(): void
    {
        ...
        $account = GroceryAccount::factory()->create(['user_id' => $user->id]);
