 ...
 */
class GrocerySyncSession
{
    private const STATE_TTL_SECONDS = 900;

    private const PASSWORD_TTL_SECONDS = 300;

    public static function start(int $groceryAccountId): string
    {
        $id = (string) Str::uuid();

        self::setState($id, ['status' => 'pending', 'grocery_account_id' => $groceryAccountId]);
    ...
    {
        return "grocery-sync-state:{$sessionId}";
    }

    private static function passwordKey(string $sessionId): string
    {
        return "grocery-sync-password:{$sessionId}";
    }
}
